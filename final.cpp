...
void quick_review()
{
	...
	cout << "Net income: $" << currentIncome << endl;

	double percentage = (currentIncome / income) * 100;
	cout << "Unspent income is " << percentage << '%' << endl;
	remaining_days();
...
string get_date(int choice)
{
	...
	else
	{
		SYSTEMTIME time;
		GetLocalTime(&time);
		date = to_string(time.wDay) + ' ' + to_string(time.wMonth) + ' ' + to_string(time.wYear);
	}
	return date;
}
